...
if (isset($_GET['DCGrupo'])) {
	...
	if(isset($_GET['fecha'])){$parametros['fecha'] = $_GET['fecha'];}
	//print_r($_GET);die();
	echo json_encode($controlador->Listar_Grupos_PV($query, $parametros));
}
if (isset($_GET['tablaClientes'])) {
	$parametros = $_POST['parametros'];
	
	echo json_encode($controlador->tablaClientes($parametros));
}
...
class facturas_distribucion
{
	...
	function DCLinea($parametros)
   {
	...
	function Listar_Grupos_PV($query, $parametros)
	{
		$datos = $this->modelo->Listar_Grupos_PV($query, $parametros);
		$res = array();
		foreach ($datos as $key => $value) {
			$res[] = array('id' => $value['Grupo'], 'text' => $value['Grupo'], 'data' => array($value));
		}
		return $res;
		// print_r($datos);die();
	}
			...
			if (strlen($FA['Autorizacion']) >= 13) {
				...
				if ($FA['TC'] <> "NDO" || $FA['TC'] <> "NDU") {
					...
					if (strlen($FA['Autorizacion']) >= 13) {
					 ...
					 return array('respuesta' => $respuesta, 'pdf' => $imp, 'clave' => $clave, 'rodillo' => $_SESSION['INGRESO']['Impresora_Rodillo']);
				}
			} else {
				// print_r('dddd');die();
				/*if ($Grafico_PV) {
