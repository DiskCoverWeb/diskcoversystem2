...
if(isset($_GET['catalogo']))
{
	...
	$datos = $controlador->getCatalogoLineas($parametros);
...
if (isset($_GET['clientes2'])) {
	...
	echo json_encode($controlador->clientes_x_grupo($query, $grupo,$cartera,$serie));
	// $estado = $_GET['estado'];
	// echo json_encode($controlador->clientes2_x_grupo($query, $grupo, $estado));
}
...
class lista_facturasC
{
	...
	function getCatalogoLineas($parametros){
		$emision = $parametros['fechaEmision'];
		$vencimiento = $parametros['fechaVencimiento'];
		$tipo = "'NDO', 'NDU'";
		...
		$datos = $this->modelo->getCatalogoLineas($emision,$vencimiento,$tipo);
		if(count($datos)==0)
		{
			return array();
		}
	...
	function grupos($query)
	{
		...
		foreach ($datos as $key => $value) {
			...
			$res[] = array('id' => $value['Fact'], 'text' => $value['Fact']);
		}
		return $res;
	}

	function clientes_x_grupo($query, $grupo,$cartera=false,$serie=false)
	{
		if ($grupo == '.') {
			$grupo = '';
		}
		$cod = '';
		$datos = $this->modelo->Cliente_facturas($cod, $grupo, $query, false, $serie);
		if(count($datos) > 0)
		{
...
QUITO - ECUADOR';
	...
	{
		$pdf = $this->ver_fac_pdf($parametros['fac'], $parametros['serie'], $parametros['codigoc'], '.', $parametros['auto'], $parametros['tc']);
