...
function costo_venta($codigo_inv,$codigoBarras=false)  // optimizado
  ...
  function grilla_generica_new($sql) {
    ...
    // Declaramos un array para almacenar los datos
    $data = [];

    // Si hay datos en la consulta, lo almacenamos
    while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
        $data[] = $row;
    }

    // Verificar si hay datos, si no, podemos enviar un mensaje vacío
    if (empty($data)) {
        $data = [];
    }
...
function FormatearNumeros($data, $SuprZero=true){
  //Nota: Usar el campo "datos o data" de tu array si estos datos estan en la etiqueta.
  //Formateo de valores contables
  foreach ($data as &$row){
    foreach($row as $key => &$value){
      $key1 = strtolower($key);
      if(strpos($key1, 'cta') || str_starts_with($key1, 'codigo')){
        continue;
      } else {
...
function Porcentajes_IVA($Fecha,$porcentaje=false,$codPorce=false){
  $conn = new db();
  $sql = "SELECT Codigo, Porc, Fecha_Inicio, Fecha_Final, ICE, IVA, ID
          FROM Tabla_Por_ICE_IVA
          WHERE IVA <> 0 
