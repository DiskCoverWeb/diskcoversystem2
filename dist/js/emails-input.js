...
console.log(email);
  ...
  function addChip(refElement, email) {
    ...
    if(to=='')
    {
      $('#txt_to').val(trimmedEmail+',');
    }else
    {
      $('#txt_to').val(to+trimmedEmail+',');
    }
  }
    ...
    addEventListener('click', function(event) {
      ...
      if (event.target.classList.contains('remove')) {
        inputContainerNode.querySelector('.emails-input')
          ...
          var to = $('#txt_to').val();
          var signo = event.target.outerText;
          var email = event.target.parentNode.innerText;

          email = email.replace(signo,'');

          var to = to.replace(email+',','');
          $('#txt_to').val(to);
