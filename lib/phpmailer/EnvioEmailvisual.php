...
class EnviarVisual 
{
	...
	function EnvioEmailVisual($parametros)
	{
		...
		if(isset($parametros['Mail_para'])){ $Mail_para = $parametros['Mail_para']; }
	        ...
	        try {
		        ...
		        $mail->Helo = 'imap.diskcoversystem.com';
			        ...
			        $mail->addReplyTo($reply, $replyName);
			          ...
			          {
			            if ($mail->send()) {
			              control_procesos("EMW", "Email: ".trim($from)."=>".trim($email_to), "Asunto: ".$parametros['subject'],'',trim($from),trim($email_to));
			              $res = 1;
			            }
			          }
